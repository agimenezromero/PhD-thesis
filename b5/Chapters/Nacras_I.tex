...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%}
        \begin{tabular}{cl:cl}
            ...
            \\ \hline
            $S$      & Susceptible host       & $\beta$   & Disease
            transmission
            rate
            \\
...
\section{Numerical analysis} \label{sec:numericalanalysis}
...
\cref{fig:Local_Sensitivity_Analysis_R0}~\textcolor{ref_color}{(a)} shows the
...
\cref{fig:Local_Sensitivity_Analysis_R0}~\textcolor{ref_color}{(b-d)}. It can
be seen that the
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figures/GSA_R_inf.png}
...
\begin{figure}[H]
    \centering
    \subfigure[Epidemic
        peak]{\includegraphics[width=0.6\textwidth]{Figures/GS_peak.png}}
    \subfigure[Time of epidemic
        peak]{\includegraphics[width=0.6\textwidth]{Figures/GS_peak_time.png}}
    \caption[
...
It can be seen that the approximation is poor when $\mu\sim
    \gamma,\bar{\beta}N$ \cref{fig:P_comparison}~\textcolor{ref_color}{(a)}, as
it could be expected.
On the other hand, the approximation is quite good when $\mu$ is one order of
magnitude larger than $\gamma$ and $\bar{\beta}N$
\cref{fig:P_comparison}~\textcolor{ref_color}{(b)},
while it is extremely accurate when $\mu$ is two orders of magnitude larger
than $\gamma,\bar{\beta}N$, \cref{fig:P_comparison}~\textcolor{ref_color}{(c)}.
...
\cref{fig:P_comparison}~\textcolor{ref_color}{(a-c)} also shows (dashed red
line) the analytical value
...
\subsection{Numerical verification of the exact reduction}
\label{sec:apprexred}
...
\cref{fig:verification_exact_reduction}~\textcolor{ref_color}{(a-c)}.
...
(\cref{fig:verification_exact_reduction}~\textcolor{ref_color}{(c)}). We recall
that in this case
...
(\cref{fig:verification_exact_reduction}~\textcolor{ref_color}{(a)}), while
\cref{fig:verification_exact_reduction}~\textcolor{ref_color}{(b)} presents the
result in an
...
\cref{fig:approx_SIR_fit}~\textcolor{ref_color}{(c-d)} shows the $RSS$ error as
$\gamma$ is varied.
