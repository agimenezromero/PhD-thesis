...
the host and can disrupt the normal functioning of the colonised organism by
...
a
wide range of models have been developed to understand the dynamics of
...
Several compartmental models have been developed to describe the dynamics of
...
% Comment on standar incidence and mass action incidence.
\subsubsection*{Incidence functions}
...
considers that rate of infection is bounded as the population grows, assuming
that the probability of a contact between a susceptible and an infected
individual is less likely as the population grows. However, other incidence
functions can be considered, such as the \textit{mass action incidence}, where
the rate of new infections is proportional to the product of the number of
susceptible and infected individuals, $\beta SI$. This incidence function can
be used to describe scenarios in which all-to-all contacts are possible
irrespective of the population size. The choice of the incidence function can
have important consequences on the dynamics of the disease, as it can affect
the threshold for the development of an epidemic, the maximum number of
infected individuals, and the final number of susceptible individuals.
...
but
one of the most common methods is the Gillespie algorithm
\cite{Gillespie1977},
...
the system can determined by two independent random processes: the time to
the
...
public health authorities to design effective strategies to prevent and
control
...
years later Charles S. Elton redefined the niche concept to encompass the roles
...
example, a species originating from a tropical region in the Americas may
have
...
competitors or predators in there. Yet, it will not be able to establish
there
if it is unable to disperse. So, in mathematical terms, the realized niche
$R$
of a species can be expressed as the intersection of the abiotic niche $A$,
the
...
the potential distribution of the species. In general, SDMs can be both
...
absence of a species to certain environmental variables. If this relation
...
conditions. Because most of the SDMs that have been employed so far are in the
...
random forests or support vector machines or MaxEnt \cite{Franklin2010}. These
...
which
uses temperature, cloud cover and leaf area index as environmental variables
to
predict the potential distribution of the banana slug using a MaxEnt model.
The
...
scenarios, such to predict the future potential distribution of species after
...
Another limitation of SDMs is that the results are highly dependent
...
the result of complex interactions occuring at multiple spatial and temporal
...
chatting with other humans or even driving autonomous vehicles.
...
success of the AlphaGo program that defeated the world champion in the game
of
Go \cite{Silver2017}.
...
\subsection{\label{sec:Artificial neural networks} Artificial neural
  networks}
...
in only one direction, from the input layer to the output layer
(\cref{fig:ANN}(a)).
...
distant parts of the time series. This is, the errors commited in the past
are
...
information while the backwards layer preserves future information.Thus,
using
the two hidden states combined BD-LSTM are able in any point in time to
preserve information from both past and future.
% A schematic representation of the BD-LSTM neural network is shown in (\cref{fig:BD_LSTM_scheme}).
...
and
fed into the network. For example, in the case of an RGB image, the input
layer
of the network would have 3 neurons, one for each color channel, and the
output
...
this task because they do not take into account the spatial information in
the
image. Indeed, we know how the brain processes images: the visual cortex is
organized in a hierarchical manner, with neurons in the lower layers
detecting
simple patterns, such as edges and textures, and neurons in the higher layers
detecting more complex patterns, such as shapes and objects. This
hierarchical
...
\begin{figure}[H]
  ...
  \caption[Convolutional Neural Network]{
    ...
    pooling layers, which apply a set of filters to the input image to
    extract
    ...
    maps, is passed through a series of non-linear activation functions,
    which
...
CNNs are a class of artificial neural networks that are specificallty
designed
...
CNNs are the basic blocks used in more complex models specidifcally design
for
...
areas of the Balearic Islands into differen benthic habitats, such as
...
Sea,
and is under a serious extinction risk due to a Mass Mortality Event (MME)
that
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Figures/Nacras.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{Figures/Xf_symptoms.pdf}
...
pattern of Ps abundance, with a peak in the spring and a decline in the
summer
...
The introduction of Xf in the Mediterranean basin has raised concerns about
its
...
mechanistic understanding on the role of environmental factors in the spread
of
plant diseases.
...
natural ecosystems for sequestering (capturing and storing) carbon,
performing
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Figures/Seagrass.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Figures/Corals.pdf}
...
Some more mechanistic hypothesis on reef formation date back to the end of
the
...
of
the industrial revolution, which corresponds to a 30\% increase in the
acidity
of the oceans \cite{Orr2005}. This trend is expected to continue in the
future
