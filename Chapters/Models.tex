...
Deep learning models have been developed for image segmentation, which are
commonly known as convolutional neural networks (CNNs). CNNs are a class of
artificial neural networks that are designed to process grid-like data, such as
images. CNNs consist of multiple layers of neurons, called convolutional
layers, which apply a set of filters to the input image to extract features.
The filters are learned during the training process and are used to detect
patterns in the image, such as edges, textures, and shapes. The output of the
convolutional layers is passed through a series of non-linear activation
functions which introduce non-linearity into the model and allow the network to
model complex patterns in the data. A schematic representation of a CNN is
shown in \cref{fig:CNN_scheme}, where we can observe how the network receives
as input an image and produces an output image with the segmented regions.
...
\begin{figure}[H]
  ...
  \caption[Convolutional Neural Network]{
    ...
    and produces an output image with the segmented regions. The network
    consists of multiple convolutional layers, which apply a set of filters to
    the input image to extract features, and non-linear activation functions,
