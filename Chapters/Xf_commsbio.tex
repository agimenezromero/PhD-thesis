...
\fullcite{GimenezRomero2022_CommsBio_e}
...
on the island of Mallorca, Spain \cite{Gomila2019, Moralejo2019}. This finding,
...
scorch disease in California \cite{Almeida2003} and Mallorca (Spain)
...
In this case it approximately $9 \times 9\,  \textrm{km}^2$, the native
resolution of ERA5-Land dataset (0.1º $\times$ 0.1º). We assume uniform
...
\cref{app:MGDD}, to obtain a piece-wise function $f(T)$
...
$CDD>306$ correspond to $T_{\textrm{min}}<\SI{-1.1}{\degree C}$
...
$\mathcal{F}(MGDD) > 99\%$ comparable to areas with high disease incidence in
    ...
    restricting PD northwards migration from the southeastern US. However,
    consistent with growing concern among Midwest states winegrowers on PD
    northwards migration led by climate change \cite{Galvez2010}, we found a
    mean
...
T_{\textrm{min}} < -1.1$ \textdegree C) since 1981, comprising land areas
    ...
    upward
    trend corresponds to $\SI{5090}{km^2}$ y$^{-1}$ in the potential northwards
...
$80\%$, except for 2006, due to data obtained from an area at the
    ...
    main wine growing regions of the Northern Hemisphere (US, Europe and China)
...
$3.5$-times greater than that of the eastern US, which could increase the
    ...
    overall very-low but localized low to moderate risk indexes in some areas
    in
...
$I(t) < I(0)$ regions increasingly displace to northern latitudes in the US
    ...
    distribution pattern holds and moves slightly northwards over time in
    ...
    2013 (Movies at \cite{Webpage}), thus preventing PD expansion northwards.
...
$\SI{114925}{ha}$ in 2020 to $\SI{43114}{ha}$ in 2050, and no vineyards would
    ...
    therefore characterized as a thermal-dependent continuous process within
...
$MGDD$ oscillations found in most locations. In addition, our model is general
    ...
    \cite{Gruber2012}, as
    has been observed at a local scale on vineyards of Mallorca
    \cite{Moralejo2019}.
